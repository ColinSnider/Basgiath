...
function Settings() {
  ...
  const dataBusy = importing || exporting || clearing;
  ...
  async function doClear() {
    ...
    try {
      await clearAll();
      setDataMsg({ ok: true, text: "Your reading data was erased." });
    } catch (error) {
      setDataMsg({
        ok: false,
        text: error instanceof Error ? error.message : "Could not erase reading data.",
      });
    } finally {
      setClearing(false);
    }
  ...
  async function onImportFile(file: File | null) {
    ...
    try {
      ...
      if (
        !confirm(
          `Replace your existing reading data with ${preview.books.length} books, ${preview.margins.length} margins, and ${preview.goals.length} goals from this file? Settings will also be replaced. This does not merge libraries. Export a backup first if you want to keep your current data.`,
        )
      )
        return;
          ...
          <aside className="space-y-4">
            <div className="rounded-[2rem] border border-gold/40 bg-gradient-to-br from-primary via-primary/90 to-gold/75 p-8 text-primary-foreground shadow-[0_24px_80px_-30px_rgba(86,25,34,0.7)]">
              ...
              <p className="mt-3 text-primary-foreground/85">
                Tune your profile, themes, reading behavior, and backups.
              </p>
              <div className="grid grid-cols-3 gap-2 text-xs mt-6">
                <div className="rounded-lg border border-gold/40 bg-black/15 p-2">
                  Account controls
                </div>
