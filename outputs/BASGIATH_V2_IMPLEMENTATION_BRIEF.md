...
- **Home is accepted as it currently exists.** Remove proposed Home redesigns, new dashboard modules, and further quick-action expansion from the remaining backlog. Preserve the current screen; fix actual defects when needed. This is not a request to revert already implemented Home code.
