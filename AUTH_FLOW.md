...
4. Server queries the `sessions` table by `id`. If no row exists, returns `null`.
5. Checks `session.expiresAt < new Date()`. If expired, returns `null`.
6. Queries the `users` table by `session.userId`, returning only public columns.
7. If the user row is missing (e.g. deleted), returns `null`.
8. On `null` response (or any error), `auth-context.tsx` removes the key from `localStorage` and leaves the user unauthenticated.
9. On a valid response, sets `user` and `sessionId` in React state — the app renders as authenticated.
...
2. `auth-context.tsx` reads `sessionId` from `localStorage` and calls the `logout` server function.
3. Server deletes the matching row from the `sessions` table (`DELETE FROM sessions WHERE id = ?`).
4. `auth-context.tsx` removes `basgiath:session` from `localStorage` and clears `user` / `sessionId` from React state.
5. The app re-renders as unauthenticated; the router redirects to `/login`.
