...
export const readingSessions = v2.table(
  "reading_sessions",
  {
    ...
    timedReads: jsonb("timed_reads")
      .$type<Array<{ startedAt: string; endedAt: string; seconds: number }>>()
      .notNull()
      .default([]),
    version: integer("version").notNull().default(0),
  },
...
export const progressEntries = v2.table(
  ...
  (t) => [
    ...
    foreignKey({
      name: "progress_replacement_session",
      columns: [t.readingSessionId, t.supersedesId],
      foreignColumns: [t.readingSessionId, t.id],
    }),
    check(
      "progress_correction_shape",
      sql`(${t.supersedesId} is null and ${t.voided} = false and ${t.correctionReason} is null) or (${t.supersedesId} is not null and ${t.supersedesId} <> ${t.id} and ${t.kind} = 'observation' and ${t.correctionReason} is not null and length(trim(${t.correctionReason})) > 0)`,
    ),
...
export const readerSeries = v2.table(
  "reader_series",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: integer("user_id")
  ...
  (t) => [
    ...
    check(
      "series_curation_text",
      sql`length(trim(${t.name})) between 1 and 120 and length(trim(${t.sourceNote})) between 1 and 1000`,
    ),
  ],
);
export const readerSeriesItems = v2.table(
  "reader_series_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: integer("user_id").notNull(),
  ...
  (t) => [
    ...
    foreignKey({
      columns: [t.userId, t.seriesId],
      foreignColumns: [readerSeries.userId, readerSeries.id],
    }).onDelete("cascade"),
    foreignKey({
      columns: [t.userId, t.userBookId],
      foreignColumns: [userBooks.userId, userBooks.id],
    }).onDelete("cascade"),
    check("series_item_order", sql`${t.sortOrder} >= 0 and length(${t.sequenceLabel}) <= 40`),
  ],
);
export const readingQueue = v2.table(
  "reading_queue",
  {
  ...
  (t) => [
    ...
    uniqueIndex("one_pinned_next_book")
      .on(t.userId)
