...
- Latest UI refinements: Calendar has its SVG icon; Insights keeps period controls in a stable row; book headers use cover-led colors with a dark contrast layer and light text; generated covers and their headers share the same title-based hue. Mobile navigation is compact with safe-area clearance outside the bar, and the Home hero has tighter mobile framing. The Shelves-only dropdown is removed and New shelf has one plus icon without the native disclosure triangle.
